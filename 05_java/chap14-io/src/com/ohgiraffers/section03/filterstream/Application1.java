package com.ohgiraffers.section03.filterstream;

import java.io.*;

public class Application1 {
    public static void main(String[] args) {
        /*
        *  BufferedWriter : 버터 임시공간을 잉요하여 성능을 향상시키는 보조스트림
        *  한 글자가 아니라 문자열 전체를 버퍼에 담아두었다가 한번에 작성한다.
        *  데이터를 일단 버퍼에 모아뒀다가, 버파가 꽉 차면 한번에 쓴다. -> 성능 향상
        * */

        // 생성자(BufferedWriter bw = new BufferedWriter)에 담아서 FileWriter를 생성한다.
        try ( BufferedWriter bw = new BufferedWriter (new FileWriter("src/com/ohgiraffers/section03/filterstream/testBuffered.txt"))) {

            bw.write("안");
            bw.write("반갑습니다.");

            //bw.flush() // 버퍼가 다 차기 전에 강제로 내보내고 싶을 때 상요
            // 지금은 try-with-resources 구문을 상요하기 때문에 flush가 자동 실행된다.
            // close()를 호출하면 내부적으로 flush()를 하고 나서 자원을 반납하기 때문.

        } catch (IOException e) {
            e.printStackTrace();
        }

        try(BufferedReader br = new BufferedReader(new FileReader("src/com/ohgiraffers/section03/filterstream/testBuffered.txt"))) {

            /* readLine() : 버퍼의 한 줄을 읽어와서 문자열로 반환*/
            String temp;
            while ((temp = br.readLine()) !=null) {
                System.out.println(temp);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
}

