...
public class Application1 {
    public static void main(String[] args) {
        ...
        * */
        ...
        // put() : 메소드를 이용하여 키-값 쌍을 지정
        hmap.put("one", "java");
        hmap.put("two", "jdbc");
        hmap.put("three", "mysql");
        hmap.put("two", "html");  //key 값이 중복되면 새로운 value로 덮어쓴다.
        hmap.put("fore","java"); // key 값이 다르고 value값이 같으면 중복 허용 {one=java, fore=java, two=html, three=mysql}

        System.out.println(hmap);  // {one=java, two=jdbc, three=mysql}
        // 키가 중복 되었을 때
        ...
        // remove(key) 메소드를 사용하여 삭제 가능
        hmap.remove("four");
        System.out.println(hmap);

        // ketSet() : 모든 키를 set으로 반환받아, 키를 하나씩 꺼내고 get()으로 값을 찾는다.
        System.out.println("keySet() 이용");
        Set<String> keys = hmap.keySet();
        Iterator<String> keyIter = keys.iterator(); // Iterator 반복자를 이용
        ...
        // entrySet() : 모든 키-값 쌍(Entry)을 Set으로 반환 받는 방법 (가장 효율적) 키-값을 하나의 객체로 취급
