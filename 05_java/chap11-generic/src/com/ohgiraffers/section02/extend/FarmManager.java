...
public class FarmManager {
    ...
    // 와일드카드<?>
    // 매개변수를 받는 것이 어떤 타입이든 Rabbit이면 받겠다
    // RabbitFarm<?> : Rabbit의 자손이면 어떤 타입이든 상관없는 RabbitFarm을 매개변수로 받는다.
    public void manageAnyFarm(RabbitFarm<?> farm) {
        System.out.print("어떤 토기 농장이든 관리 가능해요: ");
        farm.getAnimal().cry();
    }
    // Bunny이거나 Bunny의 자손 타입만 담긴 RabbitFarm만 받는다.
    ...
    public void manageRabbitOrBunnyFarm(RabbitFarm<? super Bunny> farm) {
        System.out.print("바니 혹은 일반 토기 논장에 새로운 바니를 넣어도: ");
        farm.getAnimal().cry();
    }
}
